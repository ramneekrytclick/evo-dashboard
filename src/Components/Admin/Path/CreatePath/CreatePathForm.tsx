...
const CreatePathForm = () => {
  ...
  const toggleCourseSelection = (course: CourseProps) => {
    const isSelected = selectedCourses.some((selected) => selected._id === course._id);
    ...
    <Form onSubmit={(e) => e.preventDefault()} className="needs-validation" noValidate>
              ...
              {courses.map((course) => (
                <ListGroupItem
                  ...
                  <Input
                    type="checkbox"
                    value={course._id}
                    checked={selectedCourses.some((selected) => selected._id === course._id)}
      ...
      <Row className="mt-3">
        <Col>
          ...
          />
          <Button color="primary" className="mt-2" onClick={addRoadmapEntry}>
            Add Suggestion
          </Button>
        </Col>
      </Row>
